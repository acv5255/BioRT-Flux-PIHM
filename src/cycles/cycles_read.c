...
void ReadSoilInit(const char filename[], SoilEntry *soiltbl)
{
    FILE *soil_fp;
    char cmdstr[MAXSTRING];
    int match;
    ...
    for (i = 0; i < soiltbl->number; i++)
    {
        ...
        for (k = 0; k < soiltbl->totalLayers[i]; k++)
        {
            ...
            soiltbl->clay_lyr[i][k] = (soiltbl->clay_lyr[i][k] < 0.0) ? soiltbl->clay[i] : soiltbl->clay_lyr[i][k];
            soiltbl->sand_lyr[i][k] = (soiltbl->sand_lyr[i][k] < 0.0) ? 100.0 - soiltbl->clay[i] - soiltbl->silt[i] : soiltbl->sand_lyr[i][k];
            soiltbl->iom_lyr[i][k] = (soiltbl->iom_lyr[i][k] < 0.0) ? soiltbl->om[i] : soiltbl->iom_lyr[i][k];
            soiltbl->bd_lyr[i][k] = (soiltbl->bd_lyr[i][k] < 0.0) ? soiltbl->bd[i] : soiltbl->bd_lyr[i][k];
        }
