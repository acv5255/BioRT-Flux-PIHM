...
void UpdNProf(double dt, const SoilData *soil, const wstate_struct *ws,
                  ...
                  const double solute0[], double np, double solute[])
{
    ...
    for (k = 0; k < nsoil; k++)
    {
        ...
        if (satdpth[k] > 0.0)
        {
            weight[k] = (solute[k] > 0.0) ? LinearEquilibriumConcentration(kd, bd[k], sldpth[k], swc[k],
                                                                           solute0[k]) *
                                                satdpth[k] * swc[k] * RHOH2O
                                          : 0.0;
            totwght += weight[k];
        }
        else
