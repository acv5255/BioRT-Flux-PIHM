...
double kr_func(double beta, double satn)
{
    return sqrt(satn) *
...
double field_capacity(double beta, double kv, double smcmax,
                      double smcmin)
{
    /*
...
double WiltingPoint(double smcmax, double smcmin, double alpha, double beta)
{
    ...
    return 0.5 * (smcmax - smcmin) *
               pow(1.0 / (1.0 + pow(PSIW * alpha, beta)), 1.0 - 1.0 / beta) +
           smcmin;
}
