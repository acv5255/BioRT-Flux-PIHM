...
void river_flow(MeshElement *elem, river_struct *river, int riv_mode)
{
    int i;
...
#pragma omp parallel for
    ...
    for (i = 0; i < num_river; i++)
    {
        ...
        if (river[i].down > 0)
        {
            ...
            if (river[i].attrib.riverbc_type != 0)
            {
                river[i].wf.rivflow[UP_CHANL2CHANL] +=
                    bound_flux_river(river[i].attrib.riverbc_type, &river[i].ws,
            ...
            river[i].wf.rivflow[DOWN_CHANL2CHANL] =
                channel_flow_river_to_river(&river[i], down, riv_mode);
            ...
            effk = 0.5 *
                   (effective_kh(&left->soil, left->ws.gw) +
                    effective_kh(&right->soil, right->ws.gw));
        ...
        else
        {
             ...
             */
            river[i].wf.rivflow[DOWN_CHANL2CHANL] =
                outlet_flux(river[i].down, &river[i].ws, &river[i].topo,
        ...
        river[i].wf.rivflow[CHANL_LKG] =
            channel_leak(&river[i].ws, &river[i].topo, &river[i].shp,
                         &river[i].matl);
    }
...
void RiverToElem(river_struct *river, MeshElement *left, MeshElement *right)
{
    ...
    if (river->leftele > 0)
    {
        river->wf.rivflow[LEFT_SURF2CHANL] = overland_flow_elem_to_river(left, river);
    }
    if (river->rightele > 0)
    {
        river->wf.rivflow[RIGHT_SURF2CHANL] = overland_flow_elem_to_river(right, river);
    }

    effk_left = effective_kh(&left->soil, left->ws.gw);
    effk_right = effective_kh(&right->soil, right->ws.gw);

    /* Lateral subsurface flux calculation between river-triangular element */
    if (river->leftele > 0)
    {
        river->wf.rivflow[LEFT_AQUIF2CHANL] =
            channel_flow_elem_to_river(left, effk_left, river, river->topo.dist_left);
    }
    if (river->rightele > 0)
    {
        river->wf.rivflow[RIGHT_AQUIF2CHANL] =
            channel_flow_elem_to_river(right, effk_right, river,
...
double overland_flow_elem_to_river(const MeshElement *elem, const river_struct *river)
{
    double zbank;
    double flux;
    double elem_h;
    ...
    zbank = (river->topo.zmax > elem->topo.zmax) ? river->topo.zmax : elem->topo.zmax;
...
double channel_flow_river_to_river(const river_struct *river, const river_struct *down,
                                   int riv_mode)
{
    double total_h;
    double perim;
    ...
    diff_h = (riv_mode == KINEMATIC) ? (river->topo.zbed - down->topo.zbed) : (total_h - total_h_down);
                           ...
                           const river_struct *down, double effk_nabr)
{
    ...
    avg_h = average_h(diff_h, river->ws.gw, down->ws.gw);
    distance = 0.5 * (river->shp.length + down->shp.length);
    grad_h = diff_h / distance;
...
double outlet_flux(int down, const river_wstate_struct *ws,
                   ...
                   const matl_struct *matl, const river_bc_struct *bc)
{
    ...
    case OUTLET_DIRICHLET:
        ...
        /* avg_h = average_h(grad_h, ws->stage,
        ...
        discharge =
            overland_flow(avg_h, grad_h, grad_h, crossa, matl->rough);
        break;
    case OUTLET_NEUMANN:
    ...
    case ZERO_DPTH_GRAD:
        ...
        avg_perim = RiverPerim(shp->intrpl_ord, ws->stage, shp->coeff);
        crossa = RiverCroSectArea(shp->intrpl_ord, ws->stage, shp->coeff);
        discharge = sqrt(grad_h) * crossa * ((avg_perim > 0.0) ? pow(crossa / avg_perim, 2.0 / 3.0) : 0.0) / matl->rough;
        break;
    case CRIT_DPTH:
...
double bound_flux_river(int riverbc_type, const river_wstate_struct *ws,
                        const river_topo_struct *topo, const shp_struct *shp,
                        const matl_struct *matl, const river_bc_struct *bc)
{
    double total_h;
...
double channel_flow_elem_to_river(const MeshElement *elem, double effk,
                                  const river_struct *river, double distance)
{
    ...
    avg_h = average_h(diff_h, river->ws.stage, avg_h);
...
double SubFlowElemToRiver(const MeshElement *elem, double effk,
                          const river_struct *river, double effk_riv, double distance)
{
    ...
    avg_h = average_h(diff_h, river->ws.gw, avg_h);
    aquifer_depth = river->topo.zbed - river->topo.zmin;
...
double channel_leak(const river_wstate_struct *ws, const river_topo_struct *topo,
