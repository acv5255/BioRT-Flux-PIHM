...
#if defined(_RT_)
/**
 * Data about concentration and specific surface area?
 */
typedef struct ConcSSA
{
    double t_conc[MAXSPS];
    double ssa[MAXSPS];
} ConcSSA;

typedef struct PrecipConc
{
...
/* Element structure */
typedef struct MeshElement
{
    int node_ids[NUM_EDGE];   /* nodes of triangular element
                               * (counterclockwise) */
...
#endif
#if defined(_RT_)
    ConcSSA restart_input[NCHMVOL];
    ConcSSA restart_output[NCHMVOL];
    PrecipConc prcps;
    ChemicalState chms_unsat;
    ChemicalState chms_gw;
#if defined(_FBR_)
    ChemicalState chms_fbrunsat;
    ChemicalState chms_fbrgw;
#endif
    ChemicalFlux chmf;
#endif
} MeshElement;
