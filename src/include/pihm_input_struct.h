...
/* Input file names */
typedef struct OutputFilenames
{
...
typedef struct Forcing
{
    int nbc;                 /* number of boundary condition
                              * series */
    TimeSeriesData *bc;      /* boundary condition time series */
    int nmeteo;              /* number of meteorological forcing
                              * series */
    TimeSeriesData *meteo;   /* meteorological forcing series */
    int nlai;                /* number of lai series */
    TimeSeriesData *lai;     /* lai forcing series */
    int nsource;             /* number of source forcing series*/
    TimeSeriesData *source;  /* source forcing series */
    int nriverbc;            /* number of river boundary
                              * conditions */
    TimeSeriesData *riverbc; /* river boundary condition series*/
#if defined(_BGC_)
    int nco2;
    TimeSeriesData *co2; /* CO2 forcing series */
    int nndep;
    TimeSeriesData *ndep; /* nitrogen deposition forcing
                           * series */
#endif
#if defined(_NOAH_)
    int nrad;            /* number of radiation forcing
                          * series */
    TimeSeriesData *rad; /* radiation forcing series */
#endif
#if defined(_RT_)
    ...
    TimeSeriesData *prcpc; /* concentration in precipitation */
#endif
} Forcing;

#if defined(_NOAH_)
/* Land surface parameters */
typedef struct NoahLandSurfaceEntry
{
...
#if defined(_RT_)
typedef struct ChemicalEntry
{
    double diffusion_coeff;   /* diffusion coefficient (m2 s-1) */
